...
def create_interface() -> gr.Blocks:
    ...
    # 创建界面
    with gr.Blocks(title="语音生成功能", theme=theme, css=custom_css) as app:
                        ...
                        with gr.Column(scale=1):
                            ...
                            if ASR_ENABLED:
                                ...
                                **使用方式：**
                                ...
                                - **方式二（ASR提取）**: 上传/录制音频后，点击"提取音频文字"按钮自动识别
                            ...
                            gr.Markdown(instructions_text, elem_classes=["instructions"])
...
def main():
    ...
    # 检查是否启用 share 模式（通过环境变量 GRADIO_SHARE）
    enable_share = os.environ.get('GRADIO_SHARE', 'false').lower() == 'true'

    if is_docker:
        # 在 Docker 中，根据环境变量决定是否使用 share 模式
        # 如果 GRADIO_SHARE=true，则创建公网链接；否则尝试本地模式
        app.launch(
